...
```
第1层：内存缓存 (MemoryCache) — LRU + pickle 持久化
   ↓ miss
...
- **内存缓存**：智能淘汰（5000条上限），区分稳定/易变数据，pickle 序列化到 `data/memory_cache.pkl`，每1分钟自动持久化，批量写入后即时异步持久化，启动时自动恢复
- **数据库缓存**：SQLite 存储，完整性标记，支持过期缓存降级
- **稳定数据保护**：DB验证过的缓存数据写入内存时标记为 stable（24小时TTL），淘汰时优先淘汰非稳定数据；频繁访问（≥3次）的数据自动提升为 stable

| 数据类型 | 缓存类型 | 易变TTL | 稳定TTL |
|---------|---------|--------|--------|
| 实时价格 | `price` | 交易时段30分钟 / 收盘后8小时 | 24小时（DB验证/收盘后数据） |
| OHLC走势 | `ohlc_{days}` | 交易时段30分钟 / 收盘后8小时 | 24小时（DB验证/收盘后数据） |
| 指数数据 | `index` | 交易时段30分钟 / 收盘后8小时 | 24小时（DB验证/收盘后数据） |
