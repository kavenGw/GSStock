...
个人股票管理工具（Flask + SQLAlchemy + SQLite + akshare/yfinance + 智谱GLM/Gemini + APScheduler），OCR 识别用 RapidOCR(ONNX)，行情补充数据源 同花顺 / Twelve Data / Polygon，可选 PyTorch（`app/ml/` AI 走势预测，未安装自动跳过）。
