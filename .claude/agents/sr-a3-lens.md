...
按下方「lens 选取规则」自读命中 lens 的【必查清单】节，逐条核实，查不到明写不许跳过；概念维度线索标【实证/概念】。
...
只写**一份**文件 `.omc/artifacts/<股票名>-<日期>-A3.md`（具体路径以控制者派发单为准），结构固定四段：
...
end: YYYY-MM-DD HH:MM:SS        ← 收工前再跑一次 date；字面 `end: ` 前缀，不用 HTML 注释、不用 `A3-END`
```
