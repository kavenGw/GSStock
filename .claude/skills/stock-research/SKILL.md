...
name: stock-research
description: >-
  统一投研入口，按输入形态自动识别四种模式并路由：(1) 个股深度分析/重做/重估 → 文件夹七文件 buffett 档；
  (2) 财报/业绩预告/年报分析 → 差量更新该股七文件；(3) 新闻/政策/行业事件对 docs 池的影响 → themes/ 专题档；
...
| # | 模式 | 输入形态 | 意图 | 产出 | 读 |
|---|------|----------|------|------|----|
| 1 | 深度分析 | 一只股票，**无附带材料**（或材料只是触发理由：涨价/AI/政策…） | 值不值得买 / 重估 / 重做 / 推翻旧结论 | `sectors/<sector>/<subsector>/<股票名>/` 七文件 | `references/mode-deep.md` |
| 2 | 财报分析 | 一只股票 + **财报/业绩预告/年报**（文本、PDF、URL） | 点评本体、看财报对旧 thesis 的影响 | 差量更新该股七文件（须已有文件夹档，否则升模式 1） | `references/mode-earnings.md` |
...
4. 材料是**券商路演/推票/口头观点**（不属四类材料）：单股 + 问「这话对不对」→ 模式 2 变体
   （`mode-earnings.md` §卖方口头推介变体，单路 A1 + 差量、不派 B'）；问「利好哪些票」→ 3。

开工第一句：`识别为模式 N（<一句理由>），默认参数 <...>`，用户可当场纠正。

## 歧义门（只问这些，其余按默认直接做）

1. 单股财报/纪要/卖方观点，"点评本体"还是"看池影响"意图不明 → 问一句。
