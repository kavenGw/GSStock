...
**某路用反解/倒算判定另一路口径结论对错时，控制者须用量纲不同的独立路径回算同一数再采信**——方向性假设（「追溯只会向上」）会把正确数值挂到错误口径标签上，反向证伪本来正确的一路 [L70]。
...
- 刷新价格锚后派生数 grep 扫不到，必须逐句手算 [L8]；但**核 ≠ 改**——撰写锚与回填锚并存时别逐句替换，加一张全档换算表 + 只修读作「当前」的措辞 [L32]；脚本化替换时句级与 token 级须分批跑（否则换算说明句被自我污染），并覆盖全角/半角两种乘号 [L47]：`python scripts/deep_redo_anchor_audit.py <新档文件夹> --old <旧价> --new <新价>`
  复扫须按**量纲**分货币组（元/亿/×/%）与实物组（吨/天/完成率/占比）各扫一遍，并覆盖 `related.md`/`sources.md`——它们不含估值表最易被跳过，而 `related.md` 的 note 是跨档引用唯一源 [L69]。
...
- Phase B 中断续跑给"只补缺的文件 / 只填占位、不重写已有内容"的收口棒，不重派完整写手 [L14]。
  **先 `ls -la` + `tail` 验落盘再决定**——agent 报 failed 时 result 里的「进度自述」是被杀瞬间的意图快照，"正要开始写" 常常已落盘数个完整文件 [L64]。
