...
2. 确认代码、市场（A/US/HK）、sector/subsector。前置观察取行情：验时戳与成交量非集合竞价时段 [L24]；`quote_guard` 代码带市场前缀（`sh603618`）[L36]，字段身份用兄弟股同接口交叉验、别按位置猜 [L49]；收盘后用当日收盘价作锚走 `--allow-preopen`，此时**必然**附带「竞价参考价」警告，属窗口边界，看成交量与价×股本偏差判真伪、别退回盘中价 [L43]。
...
| 判该路是否收工 | 看 evidence `end:` 戳/mtime，不看 report 是否存在；校准消息在途 ≠ 已闭合；idle 通知 ≠ 落盘完，先跑闸门 | [L1][L2][L57] |
...
| 裁定含跨主体比较派生量 | 自算算式并显式标口径（营收 / 归母 / 扣非）；A1 权威不覆盖其派生算术 | [L40][L39] |
| A2/A3 报出财务数字 | 逐条送 A1 核量级 / 科目 / 期间 / 主体，第二波裁定派 B 前收齐 | [L15] |
| 某路自报取不到关键材料 | 其派生结论整体降级，不进裁定 | [L62] |
| 裁定倚赖的硬数字 / 换算 | 亲取原文核数字倒置；换算用反解回算自检方向 | [L60][L65] |
...
| 时机 | 判据 | 出处 |
|---|---|---|
| 本轮中途下过修正单 | 派审查前逐条 grep 亲验落地 + 七文件 mtime 收敛；mtime 收敛 ≠ 在途指令已执行；审查员是快照读者，过渡态会成片假 Critical | [L53][L23][L68] |
| 审查报「全仓 0 命中」类否定断言 | 自跑 grep 复核（千分位 / 无数字表述换判据再扫）；返修单按亲验结果收窄并注明「勿按审查原话全面返工」 | [L33][L59][L61] |
| 写手自评「最脆弱假设」 | 审查核其是否真进了交付文件 | [L50] |
...
| 控制者预备 C 清单 | 对称条目按新档全部 `symmetric: true` 正推、反向链 grep 不加 `head` | [L71] |
| 清理一次性脚本 | 正向白名单只删自己建的；`_a1_*`/`_a3_*` 是各轮共用前缀，先验 mtime 归属 | [L30] |
| valuations `note` | 旧口径常以**方法论措辞**而非数字残留，自读自判 | [L67] |
| lint 绿 | `symmetric: false` 可被被检查方关掉，绿灯只证「没有开着的违规」；计数命中先排除元层面句子 | [L41][L58] |
...
| 作废口径参数 | 按它算过的每个派生数全仓 grep 复扫至归零，跨档比较两端都重述到新口径；按量纲分货币组 / 实物组各扫一遍，覆盖 `related.md`/`sources.md`；「非 X 内容仍有效」类豁免不及 X 的派生量 | [L55][L69][L66] |
...
**按 51-75min 预估**（净耗时口径；中位数 / 四分位 / 区间只看 `timing-baseline.md`，本文件不复制数字）。首建档比重做少 3-5min；
A+H 与撞财报日在中位数之上 [L9][L11][L12]；超 90min 全部由外部中断或 A 路二次返工造成，预估时问「今天会不会撞额度 / 撞财报 / 撞盘中」。
**收尾必须往 `timing-baseline.md` 加一行并跑 `scripts/timing_stats.py --check` 把统计表贴回**，预估区间变动时同步回本节。
...
- 派发内容改 `dispatch.md`；文档规格改 `buffett-doc-spec`；收尾动作改 `finalize.md` **与** `.claude/agents/sr-finalize.md`（同源两份）。改 agent 定义后只看行为验证、不问 subagent 自己 [L27]。
