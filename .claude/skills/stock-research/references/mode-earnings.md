...
- **逐条对账旧假设清单与 §11 触发器**：每条标 `证实 / 证伪 / 无信息`，证伪的给财报原文数字 + 页码/URL。
  **比率型阈值先跑上年同期回测，确认该指标对本公司有判别力再据以裁决** [L25]。
  ...
  成长股的**价格型**买点命中时先重算三档基数再谈安全边际——「跌到位」常被同一份财报带来的收入路径下修抵消 [L54]；
...
**但本模式是旧档清理的主触发场景**——新财报落地即意味着该股所有「期间数据本体」档同时过期：控制者按 `mode-deep.md`
步骤 **4b** 判据跨 `quarterly/` `themes/` `cross-sector/` 全目录扫（季报点评 / 业绩预告 / 财报联动专题 /
行情验证 / 单期事项落地档），**surface 给用户确认后**并入待删清单传给 Phase C。无命中或用户不确认 → 清单为空。
comps、机理/结构性专题档、他股主体的行业 theme 始终保留。
4b 的**承接铁律**在本模式下须**先确认差量撰写已把被删档的残值结论写进 `events.md` 正文**再列入清单——
本模式只改受财报影响的段落，容易出现「note 已更新但正文没承接」，而 note 会随条目一起消失 [L48]。
...
教训先过 `lessons.md` 新增门槛（能机械化 / 首次新形态才开新号，与模式 1 共用编号），本文件对应闸门处加 `[Ln]`。派发内容变化改 `dispatch.md`。
