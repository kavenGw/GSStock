...
## 卖方口头推介变体（材料是路演/推票/口头观点，非财报）[L72]

券商路演推票、评选拉票文案、口头观点截图**不属四模式任一**。单股主体、意图是「这话对不对」→ 走
**`SKILL.md` 歧义门 1 + 本模式变体**：单路 A1 + 差量覆盖，**不派 B'**（改动量通常只有两三段，控制者自己写更准）；
意图是「利好哪些票」→ 升模式 3。前置同本模式（须已有文件夹档）。

A1 交办在本模式 A' 清单之上换四项，**顺序不可颠倒**：

1. **先反解发言者自己的书面模型**——调出同一团队最近一份研报的预测表，把口头断言反解成模型隐含数。
   零外部信源、级别最高，对方无法申辩「我们看法不同」。**这一步没做完不许去外部找证据。**
2. **把口头断言与公司披露原文逐字对齐**——夸大多发生在转述环节（限定语被吞），原文常就在同一句里。
3. **判利益冲突窗口**（评选拉票 / 承销 / 持仓推介），并检验**偏差方向是否与其当期利益一致**，一致则折扣加重。
4. **拆开被打包的断言**逐句分裁——口头惯于把季节性常态与季节性反常并进一句，合起来裁会误判成「部分成立」。

裁定门槛：**挑战旧档评级需要「有利于该论断的、旧档采证时不存在的【硬】证据」**；
方向与旧档同向的新证据再多也不构成挑战，这一条要在结论层明写。

差量落点：卖方原文错误/口径夸大 → `sources.md` §12.3 错误清单追加编号条目；
采证暴露的旧档阈值缺陷 → `index.md` §11.2 就地订正并标注复核日期；结论摘要 → §11.2 表后一段。
**无实质新证据时不动 `conviction_date`、不动 frontmatter 任何数字字段**（价格锚未刷新，改日期会被读成重新定锚）。
