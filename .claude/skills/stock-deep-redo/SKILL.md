...
**已有基线（如实记录，不要当作已达成的目标）**：
- 2026-08-08 零跑（港股单市场、单 agent Phase A、无返修）：**~40min**
- 2026-08-09 紫金（A+H 双市场、三路并行 Phase A、含一轮 Critical 返修）：**47.0min**——
  三路并行的 Phase A 实测 12.8min，慢于零跑单 agent 的 7min。**两轮口径不可比**（标的复杂度与返修
  轮次都不同），但**至今没有任何一轮跑进 30min 以内**。派发时按 **40-47min** 预估，别按理论值许诺。
