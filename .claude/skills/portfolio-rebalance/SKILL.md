...
- 主题归属从 `docs/stock-analytics/sectors/*.md` 顶部 YAML frontmatter 读取（与 `/portfolio-init` 共享）
...
def stock_meta_lookup(code):
    ...
    if _meta_cache is None:
        universe, _ = scan_universe('docs/stock-analytics/sectors', config)
...
1. **docs/stock-analytics/ 近 7 天文档**

用 Glob 扫 `docs/stock-analytics/**/*.md`，按文件名前缀 `YYYY-MM-DD-` 解析日期，筛今日往前 7 天内。读每个文档头部 200 字，提取关键词「业绩说明会」「新增产能」「涨价」「投产」「中标」「财报联动」，形成 5-10 条事件清单。
...
| docs/stock-analytics 近 7 天无新增 | 事件清单可空，输出"近 7 天无新分析" |
