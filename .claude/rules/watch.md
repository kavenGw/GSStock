...
- **美股暗盘（盘前/盘后）**：`TradingCalendarService.get_us_extended_session()` 判 ET 04:00–09:30 `pre` / 16:00–20:00 `post`；`watch_preload` 在该时段每 3 tick 调 `get_us_extended_quotes(force_refresh=True)`（yfinance `Ticker.info` 的 `preMarket*`/`postMarket*`，仅内存缓存 `cache_type='extended'` TTL 10min，退避键 `US_EXT`）。`/watch/prices` 每条带 `ext`（非美股为 null），`/watch/market-status` 美股返回 `pre_market`「盘前」/`post_market`「盘后」，前端 `isActiveStatus` 把两者视为活跃继续 60s 轮询，summary 表涨跌% 下一行小字显示。**暗盘价不进告警/信号/AI/Slack**。SK 海力士无美股行情（OTC HXSCL 在 Yahoo 404），只用 000660.KS。
