...
环境变量（`ESPORTS_ENABLED` / `NBA_ENABLED` / `WORLDCUP_ENABLED` / `ESPORTS_*_MONITOR_INTERVAL` / `ESPORTS_PRE_MATCH_MINUTES` / `ESPORTS_FETCH_TIMEOUT`）清单与默认值见 `.env.sample`。
...
数据源：NBA / 世界杯用 ESPN API（同源同结构），LoL 用 LoL Esports API。世界杯数据层独立 `worldcup_service.py`（平局/点球/半场语义自洽），`EsportsMonitorService` 以 `match_type='worldcup'` 分支复用提醒/轮询/重试框架。

**待办**：2026 世界杯已于 2026-07 结束，`worldcup_service.py` + `worldcup_config.py` + 各处 worldcup 分支与 `CHANNEL_WORLDCUP` 尚未删除，删前按 dev-environment.md 全仓 grep importer。
