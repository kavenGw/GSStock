...
class NewsService:
    ...
    @staticmethod
    def _notify_slack(contents: list[str]):
        """Slack通知：≤3条逐条发送，>3条AI筛选关键新闻后逐条发送"""
        ...
        with app.app_context():
            ...
            try:
                if len(contents) <= 3:
                    for c in contents:
                        NotificationService.send_slack(f"📰 {c}")
                    return

                # >3条：AI筛选关键新闻
                selected = NewsService._ai_select_key_news(contents)
                if not selected:
                    # AI失败时降级：发送前3条
                    selected = contents[:3]
                for c in selected:
                    NotificationService.send_slack(f"📰 {c}")
                remaining = len(contents) - len(selected)
                if remaining > 0:
                    NotificationService.send_slack(f"📋 另有 {remaining} 条快讯，详见新闻看板")
            except Exception as e:
                logger.error(f'[新闻] Slack通知失败: {e}')

    @staticmethod
    def _ai_select_key_news(contents: list[str]) -> list[str]:
        """AI从多条新闻中筛选关键条目"""
        from app.llm.router import llm_router
        ...
        prompt = (
            f"以下是{len(contents)}条快讯，请筛选出最有价值的关键新闻（通常3-5条）。\n"
            f"返回JSON数组，包含你选中的新闻编号，如 [1, 3, 7]\n"
            f"只返回JSON数组，不要其他内容。\n\n{lines}"
        )
        ...
        try:
            ...
            import re
            response = provider.chat([
                {'role': 'system', 'content': '你是财经新闻编辑，负责从大量快讯中筛选最重要的新闻。'},
            ...
            if m:
                ...
                return [contents[i - 1] for i in indices if isinstance(i, int) and 1 <= i <= len(contents)]
        except Exception as e:
            logger.error(f'[新闻] AI筛选失败: {e}')
        return []
