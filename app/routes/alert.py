...
def refresh_signals():
    ...
    try:
        ...
        elapsed = time.time() - start_time
        logger.info(f'[alert/signals/refresh] 信号刷新完成: {updated}只, 耗时 {elapsed:.2f}s')
        return jsonify({'status': 'ok', 'updated': updated})
    except Exception as e:
        logger.error(f'[alert/signals/refresh] 信号刷新失败: {e}', exc_info=True)
        return jsonify({'error': str(e)}), 500
...
def get_earnings_data():
    ...
    try:
        logger.info(f'[alert/earnings] 开始获取 {len(stock_codes)} 只股票的财报/PE数据')
        ...
        elapsed = time.time() - start_time
        logger.info(f'[alert/earnings] 财报数据完成, 耗时 {elapsed:.2f}s')
        return jsonify(result)
    except Exception as e:
        logger.error(f'[alert/earnings] 获取财报数据失败: {e}', exc_info=True)
        return jsonify({'error': str(e)}), 500
